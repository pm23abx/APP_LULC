...
if uploaded_file:
    ...
    legend_html = '''
    <style>
        .legend-item { display: inline-block; margin-right: 15px; padding: 5px 10px; border-radius: 4px; color: white; font-weight: bold; }
    </style>
    <div>
    '''
    for i, row in class_df.iterrows():
        ...
        color = f'rgb({r},{g},{b})'
        label = row['name']
        legend_html += f'<div class="legend-item" style="background-color:{color};">{label} ({color})</div>'
    legend_html += "</div>"
